...
class MiniSWEAgent(Agent):
    ...
    def __init__(self, config: AgentConfig) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Please clone the mini-swe-agent submodule first"
            ) from exc
    ...
    def _cleanup(self) -> None:
        ...
        if self._keep_image:
            logging.getLogger("minisweagent").info("Keeping image %s", self.config.docker_image)
            return

        logging.getLogger("minisweagent").info("Removing image %s", self.config.docker_image)
